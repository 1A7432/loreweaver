...
> **传输选择**:**Iroh**(默认 · p2p · 零配置 · **富媒体图片/音频只走这条**,路线图)vs **WebSocket**(`python -m app --serve --ws`,给有域名+TLS 的常驻部署和**浏览器网页端**;**纯文本,不支持多媒体**)。有 VPS 就 `--serve --ws` 两条都开;网页端只能走 WS。协议一致,只是载体不同——详见 [docs/protocol.zh.md](docs/protocol.zh.md)。
