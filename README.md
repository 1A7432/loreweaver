...
真正的玩法:你**自托管**一台服务器,发邀请码把朋友拉进来。**朋友只用装客户端、不用读任何文档**——你把地址和邀请码发给他就行。

**① 你(房主 / 守秘人)—— 起服务端 + 拿钥匙**
```bash
./scripts/deploy.sh                              # Docker 一键起服务端(:8787)
docker exec loreweaver cat /data/keeper-key.txt  # 首次启动自动发的守秘人 key(也会打进 docker logs)
```
运行 `loreweaver`,连接屏填:主机 `wss://<你的域名>/ws` + 上面那个守秘人 key + 昵称。
...
loreweaver     # 连接屏填:主机 wss://<你的域名>/ws + 你给他的邀请码 + 昵称
```
