...
**v1.0.0 是第一个稳定版**，之后在 `1.0.1.dev*` 这条线上继续开发。这期间落了六个结构性的里程碑：
...
- **引擎韧性（M23，[spec 已公开](specs/M23-harness-resilience.md)，英文）。** 撞上上下文墙的房间会自己折叠一次再试，而不是把玩家这回合报废；进过模型的内容必须能从存档重建出来；每一种房间状态自己声明该怎么清理，而不是每个重置操作各记一张清单；定下的和否掉的设计决策也进了仓库（[docs/notes/](notes/)，英文），不再只活在某一个助手的记忆里。
