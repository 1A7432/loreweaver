...
- **也能从牌桌上装**：手上没有那台机器 shell 的守秘人，在房间里发 `.pack install <ref>`。引用形式相同，安装函数也是同一个（`gateway/pack_install.py` 同时供两道门）。在远程桌上，装上就等于启用，而且启用意味着**能直接开打**：这条命令会把包带来的开关全推上去——面板与演出包、KP 技能，以及包里恰好只有一张世界卡时的那张卡（顺带把包的建卡系统钉上）。终端里那套逐项确认在线上没有诚实的等价物，而守秘人既然把引用打了出来，信任决定就已经做过了——所以回执给的是信任卡加一行明白的风险提示，而不是一张待办清单。唯一不替人做的决定是**哪个模组**：一个包带好几张世界卡时，那是岔路不是确认，回执会把每一张写成能载入它的 `.import <ref> world`。当然是守秘人限定：它要往服务器数据目录里写。
- **Git 就是仓库**：一个安装引用可以是本地路径、`https://` 直链，或者 `gh:owner/repo[@tag]`——由 `infra/pack_source.py` 通过 GitHub API 解析到那个 release 的 `*.lwpack` asset（`@tag` 钉住一个 release，不写就是最新）。API 调用默认匿名，设了 `GITHUB_TOKEN` / `GH_TOKEN` 才带凭据，用来解掉共享机器上按 IP 的匿名限流（撞上是 403）。凭据的限域靠两条规则而不是一次主机名比对：它只跟着引擎自己拼出来的那个 release 元数据请求走（调用方自己写的 `https://api.github.com/...` 引用一律匿名取，所以守秘人的 `.pack install` 花不掉服务器的 PAT），并且跨主机重定向时会被摘掉（`urllib` 默认会把请求头原样带过去，而下载 asset 本来就要跳到别的主机）。这里刻意没有中心化包仓库。
