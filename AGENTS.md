...
- `core/` — **deterministic engine** (never AI-generated): dice, ported CoC rules, character math + creation validation, clock, `rulepacks` (data-driven rule systems), `skills` (SKILL.md loader), deterministic relationship tracks, `modvars` (deterministic module-variable trackers), worldbook, `charcard` (SillyTavern-card parser).
- `infra/` — plumbing: SQLite KV store, pydantic-settings config + hot runtime overrides/credential book, i18n, `llm`/`embeddings` (each with a `Fake*` offline double), vector, `providers` (multi-vendor LLM factory).
...
- **Content pack** → bundle a whole work (skills + rulepacks + cards + lorebooks + assets) as one `.lwpack` zip with a `pack.yaml` manifest: `python -m app --pack <src-dir>` to build, `--install <path|https|gh:owner/repo[@tag]> [--yes]` to install (Git releases ARE the registry; see `docs/plugins.md`).
