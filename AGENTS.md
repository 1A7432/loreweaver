...
- `core/` — **deterministic engine** (never AI-generated): dice, `resolution` (pack-compiled check ladders), character math + creation validation, clock, `rulepacks` (data-driven rule systems), `skills` (SKILL.md loader), deterministic relationship tracks, `modvars` (deterministic module-variable trackers), worldbook, `charcard` (SillyTavern-card parser), and `documents` — the M17 meta-type: ALL room content (lore/NPCs/sheets/pregens/modvars/MVU tree/notes/pools) is a `Document` in one table, and every type's `project(doc, viewer)` is THE single wire chokepoint for iron rule #3.
