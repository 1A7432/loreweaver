...
class MockClient implements AppClient {
  ...
  resetRoomCalls: Array<[string, string | undefined]> = []
...
describe("KeeperKeys", () => {
  ...
  test("可导出、导入并完整删除房间数据,完成后显示操作摘要", async () => {
    ...
    // The leftmost reset-scope button ("重开:仅剧情") sits at CLICK_X; two clicks
    // confirm and fire it with scope "story" (sending no backup/keys).
    const resetY = lines.findIndex((line) => line.includes("重开:仅剧情"))
    ...
    await harness.flush()
    expect(client.resetRoomCalls).toEqual([["shuxue", "story"]])
