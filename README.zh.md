...
![Loreweaver 实机演示——终端里的一场真实跑团：邀请码 p2p 连入、模组开场回放、AI 守秘人叙事、一次侦查检定真骰结算](https://raw.githubusercontent.com/1A7432/loreweaver/main/assets/demo-zh.gif)
...
**不配 API key 也能先尝一口**：没配模型时，内置的演示守秘人会接管应答——骰子照样是真的——等你在模型页配好真模型再换。
...
服务器程序也可以自己拿去服务器上长期部署：[GitHub Releases](https://github.com/1A7432/loreweaver/releases) 上有 Windows / macOS / Linux(x64 + arm64)四份 `loreweaver-server-*`，解压即跑，`--doctor` 一键体检。
...
不需要域名、证书、端口转发这些东西。连接走的是 p2p（Iroh，QUIC 直连，打洞失败自动走中继，端到端加密）；ticket 存在你本地，重启也不会变，发一次以后一直用。没有账号注册这回事，邀请码本身就是入场券；想要个副 KP 的话，再发一把守秘人角色的钥匙就行。掉线会自动重连，回来接着玩。
...
<p align="center">
  <img src="https://raw.githubusercontent.com/1A7432/loreweaver/main/assets/tui-connect-zh.png" width="49%" alt="连接屏：一键本地开服、已存服务器、ticket 登录" />
  <img src="https://raw.githubusercontent.com/1A7432/loreweaver/main/assets/tui-character-zh.png" width="49%" alt="建卡：四种方式，手动模式实时校验点数预算" />
</p>
<p align="center">
  <img src="https://raw.githubusercontent.com/1A7432/loreweaver/main/assets/tui-menu-zh.png" width="49%" alt="守秘人主菜单：房间与邀请、导入模组、规则系统、KP 技能、模型配置" />
  <img src="https://raw.githubusercontent.com/1A7432/loreweaver/main/assets/tui-skills-zh.png" width="49%" alt="KP 技能页：开关玩法包，描述一句话生成新技能" />
</p>
...
- AI 是真的在带团，不是陪聊。掷骰、翻角色卡、记笔记、推时钟，这些都是它通过 60 多个守秘人工具实际操作引擎完成的。接哪家模型都行，推荐 `deepseek-v4-pro` 开思考。
- NPC 不开天眼。每个 NPC 和 AI 队友只知道自己该知道的事，剧情底牌根本不在它们手里，想剧透都剧透不了。缺人的时候 AI 队友真的能顶一个位置，用自己的卡掷自己的骰。
...
测试有 955 个，全程离线，结果可复现。其中一组自 play 测试会用脚本化的 KP 把整条链路跑一遍（传模组 → 分析 → 开团 → 玩家行动 → 真骰检定 → 战报）；像"秘密进不了玩家池""NPC 只能由自己的档案组装"这类底线，都有专门的红线测试看着。

离线测试只能证明流程对；真模型守不守规矩是另一回事，所以另外有一道每夜跑的真模型评测（`.github/workflows/redline-eval.yml`）：让脚本化玩家陪真模型跑几十个回合，逐回合统计"剧透率"和"该掷骰没掷率"，超了阈值就报红。这道闸门刚架起来的时候抓到过真问题：KP 在战斗叙事和战斗表格里管不住嘴，泄密率一度 45.8%，修了六轮才稳定在 0，现在每天盯着防退化。它只按计划跑、不卡 PR；没配 `EVAL_LLM_API_KEY` 时自动跳过。CI(push/PR)跑 Python 3.11/3.12 和各客户端包，全程离线。
...
没有账号系统：邀请码就是通行证，它把玩家绑到某个房间、带玩家或守秘人角色。要面向可信小圈子以外开放，请自己在前面加鉴权和 TLS，别直接裸奔公网，这是任何自托管服务都该有的卫生习惯。
