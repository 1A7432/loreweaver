...
**不用买域名、不用配证书、不用开端口转发。** 连接走 p2p（Iroh，QUIC 直连，打不通走中继，端到端加密）;ticket 存在你本地，重启也不变，发一次一直能用。没有注册这回事，邀请码就是入场券；想要个副 KP，就发一把守秘人角色的钥匙。掉线会自动重连，回来接着玩。

## 它凭什么不一样

现有的工具就两类：骰子机器人（SealDice、Avrae）骰子很硬，可没人带团；角色扮演聊天（SillyTavern/酒馆）聊得热闹，可没有规则、没有世界，你永远不会失败。Loreweaver 把两边都缺的那块补上了：
