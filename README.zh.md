...
## 快速上手
```bash
uv sync                     # 建 .venv + 装依赖（含 dev 工具）。原生 provider：uv sync --extra anthropic --extra gemini

# 1) 离线——无需 API key（内置演示 Keeper + 确定性骰子）：
uv run python -m app --cli              # REPL：试 r 3d6+2  /roll 4d6kh3  .ra 侦查  .setcoc 2
uv run python -m app --cli --script tests/fixtures/selfplay_en.txt   # 离线 AI-KP 自play demo
...
uv run python -m app --cli              # 自然语言回合现在由真 Keeper 跑

# 没有 uv？pip 也行：python3 -m venv .venv && . .venv/bin/activate && pip install -e ".[dev,anthropic,gemini]"
```
...
## 测试
```bash
uv run pytest -q                            # 离线测试（FakeLLM/FakeEmbeddings、seed 骰子）
uv run ruff check core infra agent gateway net adapters app.py scripts
uv run python scripts/i18n_lint.py          # 无硬编码自然语言串
